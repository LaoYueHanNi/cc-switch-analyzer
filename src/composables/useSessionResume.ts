...
export type ResumableSourceType = 'claude' | 'opencode' | 'codex' | 'grokbuild' | string | undefined
...
export function useSessionResume(logTag: string, onError?: (label: string, err: any) => void) {
  async function resumeSession(sourceType: ResumableSourceType, sessionId: string, projectDir?: string): Promise<void> {
    const type =
      sourceType === 'opencode' || sourceType === 'codex' || sourceType === 'grokbuild'
        ? sourceType
        : 'claude'
    const label = RESUME_LABELS[type] || type
    try {
      ...
      else if (type === 'grokbuild') await platformAdapter.resumeGrokSession(sessionId, projectDir)
