...
impl ExternalDbService {
    ...
    pub fn get_recent_request_logs_raw(&self, since: Option<i64>) -> Result<Vec<crate::services::data_source::StreamingRecord>, String> {
        ...
        let mut stmt = db.prepare(&sql).map_err(|e| format!("准备查询最近请求日志失败: {}", e))?;
...
impl super::data_source::DataSource for ExternalDbService {
    ...
    fn get_recent_request_logs_raw(&self, since: Option<i64>) -> Result<Vec<crate::services::data_source::StreamingRecord>, String> { self.get_recent_request_logs_raw(since) }
    fn stream_records(&self, since: Option<i64>, on_record: &mut dyn FnMut(crate::services::data_source::StreamingRecord)) -> Result<(), String> { self.stream_records(since, on_record) }
