...
//! source='Antigravity' 的记录(由 antigravity_scanner 经本地 language server
//! RPC 拉取后入库)。聚合复用 pipeline::aggregate_*,与 DSH / MiniMax 同为
//! 「SQL 取记录 + 内存聚合」的扫描入库型数据源。
...
pub struct AntigravityDbService {
    db: Option<Mutex<Connection>>,
    db_path: String,
}

impl AntigravityDbService {
    pub fn new() -> Self {
        Self {
            db: None,
            db_path: String::new(),
    ...
    pub fn open(&mut self, file_path: &str) -> Result<(), String> {
        ...
        .map_err(|e| {
            log::error!("[ANTIGRAVITY] 打开数据库失败 (path={}): {}", file_path, e);
            "打开 Antigravity 数据库失败,请检查应用库路径".to_string()
        })?;
    ...
    fn db(&self) -> Result<std::sync::MutexGuard<'_, Connection>, String> {
        self.db
            .as_ref()
            .ok_or_else(|| "Antigravity 数据库未打开".to_string())?
            .lock()
            .map_err(|e| format!("Antigravity 数据库锁失败: {}", e))
    }

    /// 按 FilterParams 过滤查询 Antigravity 记录(SQL 取全量 + 内存过滤)
    fn filtered_records(&self, params: &FilterParams) -> Result<Vec<RawRecord>, String> {
        ...
        let mut stmt = db
            ...
            .map_err(|e| format!("查询 Antigravity 记录失败: {}", e))?;
        let rows = stmt
            .query_map([], |row| {
                Ok(RawRecord {
        ...
        let mut out = Vec::new();
        for r in rows {
            let rec = r.map_err(|e| format!("读取 Antigravity 记录失败: {}", e))?;
...
impl DataSource for AntigravityDbService {
    ...
    fn get_record_count(&self) -> Result<i64, String> {
        let db = self.db()?;
        db.query_row(
            "SELECT COUNT(*) FROM session_request_logs WHERE source = 'Antigravity'",
            [],
            |row| row.get(0),
        )
        .map_err(|e| format!("查询 Antigravity 记录数失败: {}", e))
    }
    ...
    fn get_latest_timestamp(&self) -> Option<i64> {
        ...
        self.db()
            ...
            .and_then(|db| {
                db.query_row(
                    "SELECT MAX(created_at) FROM session_request_logs WHERE source = 'Antigravity'",
                    [],
        ...
        let mut stmt = db
            .prepare(
                "SELECT DISTINCT model FROM session_request_logs
                 WHERE source = 'Antigravity' AND model <> '' ORDER BY model",
            )
            .map_err(|e| format!("查询 Antigravity 模型失败: {}", e))?;
        let rows = stmt
            .query_map([], |row| row.get::<_, String>(0))
            .map_err(|e| format!("查询 Antigravity 模型失败: {}", e))?;
        let mut out = Vec::new();
        for r in rows {
            out.push(r.map_err(|e| format!("读取 Antigravity 模型失败: {}", e))?);
        }
        Ok(out)
    }
        ...
        let (min, max) = db
            .query_row(
                "SELECT MIN(created_at), MAX(created_at)
                 FROM session_request_logs WHERE source = 'Antigravity'",
                [],
                |row| {
                    Ok((
    ...
    // 会话 tab 暂不接入(capabilities.session_management = false),早期返回空
    ...
    fn get_recent_request_logs_raw(
        ...
        let mut stmt = db
            .prepare(&sql)
            .map_err(|e| format!("查询 Antigravity 最近请求日志失败: {}", e))?;
        ...
        let mut out = Vec::new();
        for r in rows {
            out.push(r.map_err(|e| format!("读取 Antigravity 最近请求日志失败: {}", e))?);
        }
        Ok(out)
    }
...
mod tests {
    ...
    /// 建一张带 session_request_logs 表的临时应用库(先建库插入再只读打开)
    fn temp_app_db() -> (std::path::PathBuf, String) {
        let path = std::env::temp_dir().join(format!(
            "ccsa_antigravity_db_test_{}_{}.db",
            std::process::id(),
        ...
        let mut insert = |request_id: &str,
            ...
            conn.execute(
                ...
                rusqlite::params![
                    request_id, source, session, model, provider, input, output, cache_read,
                    created_at, latency
                ],
            )
            .unwrap();
    ...
    #[test]
    fn reads_only_antigravity_rows() {
        let (path, path_str) = temp_app_db();
        let mut svc = AntigravityDbService::new();
        svc.open(&path_str).unwrap();

        assert_eq!(svc.get_record_count().unwrap(), 2);
        assert_eq!(svc.get_models().unwrap(), vec!["gemini-3.8-flash"]);
        let range = svc.get_date_range().unwrap();
        assert_eq!((range.min, range.max), (1000, 2000));
    ...
    fn recent_request_logs_raw_orders_desc_and_filters_since() {
        ...
        assert_eq!(all[0].6, 32677);
        assert_eq!(all[0].7, 0);
        assert_eq!(all[0].8, 1800);

        let fresh = svc.get_recent_request_logs_raw(Some(2000)).unwrap();
    ...
    #[test]
    fn summary_aggregates_tokens_and_respects_time_filter() {
        let (path, path_str) = temp_app_db();
        let mut svc = AntigravityDbService::new();
        svc.open(&path_str).unwrap();
        ...
        let summary = svc.get_summary(&all).unwrap();
        assert_eq!(summary.total_requests, 2);
        assert_eq!(summary.total_input, 15522 + 15508);
        assert_eq!(summary.total_output, 135 + 214);
        assert_eq!(summary.total_cache_read, 32677);

        let windowed = FilterParams {
            from_epoch: Some(1500),
            ..all
        };
        assert_eq!(svc.get_summary(&windowed).unwrap().total_requests, 1);
