//! DSH 数据源(只读)
//!
//! 以只读连接打开应用自有库 pricing.db,从 session_request_logs 中读取 source='dsh'
//! 的记录(由 dsh_scanner 扫描入库)。聚合复用 pipeline::aggregate_*(与 Proma 相同),
...
impl DshDbService {
    ...
    pub fn open(&mut self, file_path: &str) -> Result<(), String> {
        ...
        .map_err(|e| {
            log::error!("[DSH] 打开数据库失败 (path={}): {}", file_path, e);
            "打开 DSH 数据库失败,请检查应用库路径".to_string()
        })?;
    ...
    fn db(&self) -> Result<std::sync::MutexGuard<'_, Connection>, String> {
        self.db
            .as_ref()
            .ok_or_else(|| "DSH 数据库未打开".to_string())?
            .lock()
            .map_err(|e| format!("DSH 数据库锁失败: {}", e))
    }

    /// 按 FilterParams 过滤查询 DSH 记录(SQL 取全量 source='dsh' + 内存过滤)。
    fn filtered_records(&self, params: &FilterParams) -> Result<Vec<RawRecord>, String> {
        ...
        let mut out = Vec::new();
        for r in rows {
            let rec = r.map_err(|e| format!("读取 DSH 记录失败: {}", e))?;
...
impl DataSource for DshDbService {
    ...
    fn get_record_count(&self) -> Result<i64, String> {
        ...
        .map_err(|e| format!("查询 DSH 记录数失败: {}", e))
    }
    ...
    fn get_models(&self) -> Result<Vec<String>, String> {
        ...
        let mut out = Vec::new();
        for r in rows {
            out.push(r.map_err(|e| format!("读取 DSH 模型失败: {}", e))?);
        }
        Ok(out)
    }
    ...
    fn get_recent_request_logs_raw(
        ...
        let mut stmt = db
            .prepare(&sql)
            .map_err(|e| format!("查询 DSH 最近请求日志失败: {}", e))?;
        ...
        let mut out = Vec::new();
        for r in rows {
            out.push(r.map_err(|e| format!("读取 DSH 最近请求日志失败: {}", e))?);
        }
        Ok(out)
    }
...
mod tests {
    ...
    fn temp_app_db() -> (std::path::PathBuf, String) {
        ...
        conn.execute(
            ...
            rusqlite::params![
                "r-old", "s1", "deepseek-chat", "DSH", 100, 200, 50, 10, 1000, 300,
            ],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO session_request_logs
             ...
             VALUES (?1, 'DSH', ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9, ?10)",
            rusqlite::params![
                "r-new", "s1", "deepseek-reasoner", "DSH", 400, 500, 60, 20, 2000, 900,
            ],
        )
        .unwrap();
        // 其他源的记录不应返回
        conn.execute(
            ...
            rusqlite::params![
                "r-mm", "s9", "mm-model", "MiniMax", 1, 1, 0, 0, 1500, 10,
            ],
        )
        .unwrap();
        drop(conn);
