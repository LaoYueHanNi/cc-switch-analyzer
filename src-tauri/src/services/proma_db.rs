...
impl DataSource for PromaDbService {
    ...
    fn get_recent_request_logs_raw(
        ...
        let mut stmt = db
            .prepare(&sql)
            .map_err(|e| format!("查询 Proma 最近请求日志失败: {}", e))?;
        ...
        let mut out = Vec::new();
        for r in rows {
            out.push(r.map_err(|e| format!("读取 Proma 最近请求日志失败: {}", e))?);
        }
        Ok(out)
    }
...
mod tests {
    ...
    fn temp_app_db() -> (std::path::PathBuf, String) {
        ...
        conn.execute(
            "INSERT INTO session_request_logs
             ...
             VALUES (?1, 'Proma', ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9, ?10)",
            rusqlite::params![
                "r-old", "s1", "proma-model", "Proma", 100, 200, 50, 10, 1000, 300,
            ],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO session_request_logs
             ...
             VALUES (?1, 'Proma', ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9, ?10)",
            rusqlite::params![
                "r-new", "s1", "proma-model-2", "Proma", 400, 500, 60, 20, 2000, 900,
            ],
        )
        .unwrap();
        // 其他源的记录不应返回
    ...
    fn recent_request_logs_raw_orders_desc_and_filters_since() {
        ...
        let mut svc = PromaDbService::new();
